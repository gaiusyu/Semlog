...
def train(path,epoch_n,output,type,number):
    ...
    for epoch in range(epoch_n):
        ...
        for input_ids, masked_tokens, masked_pos in loader:
            ...
            optimizer.step()
        if epoch %5==0:
           torch.save(model.state_dict(), '../SaveFiles&Output/modelsave/ANDROIDDDDDDDmodel' + str(output) + 'epoch' + str(epoch))
    torch.save(model.state_dict(), '../SaveFiles&Output/modelsave/ANDROIDDDDDDDmodel'+ str(output))
...
def train_parse(type,cluster_list):
    ...
    for number in line.split('\n'):
        if os.path.exists(str('../SaveFiles&Output/Cluster/')+ type + '/'  + str(type) + number + '.csv') == True:
            print(number)
            train(str('../SaveFiles&Output/Cluster/')+ type + '/'  + str(type) + number + '.csv', 10,str(type)+number,str(type),number)
...
def parse_certain_cluster(type,i):
        if os.path.exists(str('../SaveFiles&Output/Cluster/')+ type + '/'  + str(type) + str(i) + '.csv') == True:
            ...
            parse('modelsave/Sparksave/ANDROIDDDDDDDmodelSpark99',
...
def clusteringwithlen(content_path, output):
        ...
        for sentence in sentences:
            ...
            f.close()
            n+=1
def get_Percentage(input_path,type,percentage):
    lines = open(input_path)  # 加载embedding
    line = lines.read()
    lines.close()
    sentences = line.split('\n')
    count=int(percentage*len(sentences))
    index=random.sample(range(0,len(sentences)),count)
    with open('../SaveFiles&Output/Cluster/' + type + '/'+type+str(percentage)+'P.csv', 'w') as f:
        for i in index:
            f.write(sentences[i]+'\n')
        f.close()
'''
...
def parse_graph(type, i):
    if os.path.exists(str('../SaveFiles&Output/Cluster/') + type + '/' + str(type) + str(i) + '.csv') == True:
        print(i)
        contrbution,input_ids=acc_parse('../SaveFiles&Output/modelsave/ANDROIDDDDDDmodelAndroid'+str(i)+'epoch15',
              str('../SaveFiles&Output/Cluster/') + type + '/' + str(type) + str(i) + '.csv', type, str(i))
